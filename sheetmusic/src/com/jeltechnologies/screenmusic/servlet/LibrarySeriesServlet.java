...
public class LibrarySeriesServlet extends BaseServlet {
	...
	try {
		...
		responseJSON = new Library(getUser(request), new ScreenMusicContext(request)).getBooksInSeries(series);
	    } else {
		responseJSON = new Library(getUser(request), new ScreenMusicContext(request)).getSeries();
	    }
